...
package com.example.spasdomworkerapp.tinydb;
...
public class TinyDB {
    ...
    public void putListPhoto(String key, ArrayList<SavingPhoto> photoList){
        checkForNullKey(key);
        Gson gson = new Gson();
        ArrayList<String> objStrings = new ArrayList<String>();
        for(SavingPhoto photo: photoList){
            objStrings.add(gson.toJson(photo));
        }
        putListString(key, objStrings);
    }

    //No need Class<?> mClass parameter. Because we know it is Player!
    public ArrayList<SavingPhoto> getListPhoto(String key){
        Gson gson = new Gson();

        ArrayList<String> objStrings = getListString(key);
        ArrayList<SavingPhoto> photoList =  new ArrayList<SavingPhoto>();

        for(String jObjString : objStrings){
            SavingPhoto photo  = gson.fromJson(jObjString,  SavingPhoto.class);
            photoList.add(photo);
        }
        return photoList;
    }
